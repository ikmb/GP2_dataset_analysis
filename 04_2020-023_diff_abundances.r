...
#by group
rg_BSPSPC <- rarefied_genus[rarefied_genus$Group == 'BSPSPC', ]
rg_BSPSPC <- subset(rg_BSPSPC, select = -c(Material, Group, Combined))
meta_BSPSPC <- meta_df_sub[meta_df_sub$Group == 'BSPSPC', ]


#Create tse object
count_matrix <- as.matrix(t(rg_BSPSPC))
counts <- S4Vectors::SimpleList(counts = count_matrix)
metadata= S4Vectors::DataFrame(meta_BSPSPC)
...
write.csv(tab_zero, file = "./results/2020-023_tab_zero_BSPSPC.csv", row.names = TRUE)

res_prim  <-  output$res
write.csv(res_prim, file = "./results/2020-023_res_prim_BSPSPC.csv", row.names = TRUE)

res_global  <-  output$res_global
write.csv(res_global , file = "./results/2020-023_res_global_BSPSPC.csv", row.names = TRUE)

res_pair  <-  output$res_pair
write.csv(res_pair, file = "./results/2020-023_res_pair_BSPSPC.csv", row.names = TRUE)

res_dunn  <-  output$res_dunn
write.csv(res_dunn, file = "./results/2020-023_res_dunn_BSPSPC.csv", row.names = TRUE)

res_trend  <-  output$res_trend
write.csv(res_trend, file = "./results/2020-023_res_trend_BSPSPC.csv", row.names = TRUE)